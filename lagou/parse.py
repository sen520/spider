...
pattern = '\d+'
df['workYear'] = df['workYear'].str.findall(pattern)

# 数据处理后的工作年限
avg_work_year = []
# 工作年限
for i in df['workYear']:
...
df['招聘学历'] = df['education'].replace('不限', '大专')
df['招聘学历'] = df['education'].replace('学历不限', '大专')
...
# 绘制词云,将职位福利中的字符串汇总
text = ''
label_df = df.fillna(value={'companyLabelList': ''})
for line in label_df['companyLabelList']:
    text += ''.join(line.split(', '))
cut_text = ' '.join(jieba.cut(text))
color_mask = plt.imread('python.jpg')  #设置背景图
cloud = WordCloud(
